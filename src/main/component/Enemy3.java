...
public class Enemy3 extends JLabel implements Moveable {
	...
	// 적 속도 상태
	private final int SPEED = 1; // 상수는 대문자로 선언한다.
	private final int JUMPSPEED = 1;

	// 왼쪽, 오른쪽 방향 이미지
	private ImageIcon enemyR, enemyL;

	public Enemy3(FrameMain mContext, EnemyWay enemyWay) {
	...
	private void initObject() {
		enemyR = new ImageIcon("image/monsterBallokR.png");
		enemyL = new ImageIcon("image/monsterBallokL.png");
	}
...
//		System.out.println("left");
		...
		new Thread(() -> { // 람다식 표현 ()->{}, 익명 Runnable 인터페이스에 있는 run 추상메소드를 간소화 표현.
			while (left) {
				...
				if (Math.abs(x - player.getX()) < 270 && Math.abs(y - player.getY()) < 270) {
					if (player.getState() == 0 && getState() == 0)
						player.die();
					System.out.println("ballok 플레이어 죽임");
				}
...
//		System.out.println("점프");
		up = true;
		new Thread(() -> {
			for (int i = 0; i < 130 / JUMPSPEED; i++) { // JUMSPEED의 값에 따라 이동거리가 달라짐으로 160을 JUMPSPEED로 나눠주면
