...
public class Enemy2 extends JLabel implements Moveable {
	...
	public Enemy2(FrameMain mContext, EnemyWay enemyWay) {
	...
	private void initObject() {
		enemyR = new ImageIcon("image/monsterSlimeR.png");
		enemyL = new ImageIcon("image/monsterSlimeL.png");
	}
			...
			while (left) {
				...
				if (Math.abs(x - player.getX()) < 50 && Math.abs(y - player.getY()) < 50) {
					if (player.getState() == 0 && getState() == 0)
						player.die();
					System.out.println("slime 플레이어 죽임");
				}
